    ...
    >
      ...
      <el-table-column v-if="showCover" label="封面" width="150" align="center">
        <template slot-scope="{ row }">
          <a :href="targetCoverPath(row)" target="_blank">
            <img
              :src="targetCoverPath(row)"
...
export default {
  ...
  methods: {
    targetCoverPath(data) {
      if (+data.updateType === 0) {
        return data.cover.replace("localhost:8089", "www.youbaobao.xyz");
      } else {
        return data.cover;
      }
    },
    ...
    refresh() {
      this.$router.push({
        ...
        query: {
          ...this.listQuery,
          date: new Date().getTime(), // 解决不刷新的问题
        },
      });
    },
    handleFilter() {
